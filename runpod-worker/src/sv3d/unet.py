...
from diffusers.models.attention_processor import AttentionProcessor, CROSS_ATTENTION_PROCESSORS, AttnProcessor


class SV3DUNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    A conditional Spatio-Temporal UNet model for SV3D that takes noisy video frames,
    conditional state, and a timestep and returns a sample shaped output.

    This model is modified from SVD to support camera angle conditioning (polar, azimuth).
    """
    ...
    def __init__(
        ...
        # SV3D-specific: encode camera angles (polar and azimuth)
        self.add_angle_proj = Timesteps(2 * addition_time_embed_dim, True, downscale_freq_shift=0)
    ...
    def forward(
        ...
        r"""
        The SV3DUNetSpatioTemporalConditionModel forward method.

        Args:
            sample (`torch.Tensor`):
                The noisy input tensor with shape `(batch, num_frames, channel, height, width)`.
            timestep (`torch.Tensor` or `float` or `int`): The number of timesteps to denoise an input.
            encoder_hidden_states (`torch.Tensor`):
                The encoder hidden states with shape `(batch, sequence_length, cross_attention_dim)`.
            added_time_ids: (`torch.Tensor` or `List[torch.Tensor]`):
                For SV3D, this is a list of [cond_aug, polars, azimuths] tensors.
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a UNetSpatioTemporalConditionOutput instead of a plain tuple.
        Returns:
            UNetSpatioTemporalConditionOutput or tuple
        """
        # 1. time
        timesteps = timestep
        ...
        if isinstance(added_time_ids, torch.Tensor):
            ...
            # SV3D-specific: handle [cond_aug, polars, azimuths] format
            # Repeat the embeddings num_video_frames times
            emb = emb.repeat_interleave(num_frames, dim=0)
            ...
            raise ValueError("added_time_ids must be a Tensor or a list of [cond_aug, polars, azimuths]")
