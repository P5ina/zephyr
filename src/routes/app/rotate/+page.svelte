<script lang="ts">
	...
	function calculateAngle8(
		source: SourceDirection8,
		target: Direction8,
	): number {
						...
						{#if job.status === 'completed'}
							{#if getJobPreviewImage(job)}
								<img
									...
									alt="Rotation"
									class="h-full w-full bg-zinc-800 object-contain"
								/>
							{:else}
								<div
	...
	<div class="grid flex-1 grid-cols-1 gap-5 lg:grid-cols-2">
		...
		<div class="panel">
			{#if viewMode === 'new'}
				...
				{#if previewUrl}
					<div
						class="relative mb-4 aspect-square overflow-hidden rounded-xl border border-zinc-700/50 bg-zinc-800/40"
					>
						<img
							src={previewUrl}
						...
						<button
							...
							class="absolute top-2 right-2 rounded-lg bg-black/50 p-1.5 backdrop-blur-sm transition-colors hover:bg-black/70"
						>
				...
				{:else}
					...
					>
						...
						>
							<Upload class="mb-3 h-10 w-10 text-zinc-500" />
							<p class="mb-1 text-center text-sm text-zinc-400">
				...
				{#if sprites.length > 0}
					<div class="relative mb-4">
						<button
							...
							class="flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-700/50 bg-zinc-800/40 px-4 py-2.5 text-sm text-zinc-300 transition-colors hover:bg-zinc-800/60"
						>
									...
									{#each sprites as sprite (sprite.id)}
										...
										{#if url}
											<button
												...
												class="aspect-square overflow-hidden rounded-lg border border-zinc-700/50 bg-zinc-800 transition-colors hover:border-amber-500/40"
											>
												<img
													src={url}
													alt={sprite.prompt}
				...
				<div class="mb-4">
					<div class="mb-2 flex items-center justify-between">
						<label for="elevation" class="text-sm text-zinc-400"
							>Camera Elevation</label
						>
					...
					<input
						...
						max="90"
						step="5"
						class="h-2 w-full cursor-pointer appearance-none rounded-lg bg-zinc-700 accent-amber-500"
					/>
			...
			{:else if selectedJob}
				...
				{#if selectedJob.status === 'processing' || selectedJob.status === 'pending'}
					<div
						class="mb-4 flex aspect-square flex-col items-center justify-center rounded-xl border border-zinc-700/40 bg-zinc-800/25"
					>
				...
				{:else if selectedJob.status === 'failed'}
					<div
						class="mb-4 flex aspect-square flex-col items-center justify-center rounded-xl border border-red-500/15 bg-red-500/5 p-6"
					>
				...
				{:else if selectedJob.status === 'completed'}
					{#if selectedJob.inputImageUrl}
						<div class="mb-4">
							<span class="mb-2 block text-xs text-zinc-500">Input Image</span>
							<div
					...
					>
						<div class="flex items-center justify-between text-sm">
							...
							<span class="font-medium text-white"
								>{selectedJob.elevation ?? 20}&deg;</span
							>
						</div>
					</div>
				...
				{#each directions as dir (dir.key)}
					{#if dir.key === 'center'}
						<div class="dir-cell dir-cell-center">
							<RotateCw class="h-8 w-8 text-zinc-600" />
						</div>
					{:else}
						{@const url =
							displayRotations[dir.key as keyof typeof displayRotations]}
						<button
							onclick={() => {
								if (url) {
									const idx = animationOrder.indexOf(
										dir.key as (typeof animationOrder)[number],
									);
									if (idx !== -1) {
										viewerDirection = idx;
										showViewer = true;
						...
						>
							{#if regeneratingDirection === dir.key}
								<div
									class="flex h-full w-full flex-col items-center justify-center bg-zinc-800/80"
								>
								...
								<div class="dir-cell-hover">
									...
									{#if !data.isGuest && selectedJob?.status === 'completed'}
										<button
											onclick={(e) => {
												e.stopPropagation();
												openRegenerateModal(dir.key as Direction8);
											}}
											disabled={regenerating}
											class="dir-action-gold"
											title="Regenerate this view"
										>
											<RefreshCw class="h-5 w-5 text-zinc-900" />
										</button>
									{/if}
								</div>
							...
							{:else}
								{@const Icon = dir.icon}
								<Icon class="mb-1 h-6 w-6 text-zinc-600" />
...
{#if showViewer}
	<div
		...
		tabindex="-1"
	>
		<div class="relative flex max-h-full w-full max-w-3xl flex-col">
			<button
				onclick={closeViewer}
				class="absolute -top-2 -right-2 z-10 rounded-full bg-zinc-800 p-2 text-zinc-400 transition-colors hover:bg-zinc-700 hover:text-white"
			>
				<X class="h-5 w-5" />
			</button>
			...
			<div class="viewer-shell">
				...
				<div class="viewer-controls">
					<div class="flex items-center justify-center gap-1">
						{#each animationOrder as dir, i (dir)}
							{@const url =
								displayRotations[dir as keyof typeof displayRotations]}
							<button
								onclick={() => {
									viewerDirection = i;
								}}
								class="viewer-thumb {viewerDirection === i
									? 'viewer-thumb-active'
									: ''}"
							>
								{#if url}
								...
								{:else}
									<div
										class="flex h-full w-full items-center justify-center bg-zinc-800 text-[8px] text-zinc-500"
									>
										{dir.toUpperCase()}
									</div>
								{/if}
					...
					<div class="flex flex-wrap items-center justify-center gap-3">
						...
						<div class="h-6 w-px bg-zinc-700/50"></div>
					...
					<div
						class="border-t border-zinc-800/40 pt-2 text-center text-xs text-zinc-500"
					>
						<span
							class="inline-flex flex-wrap items-center justify-center gap-4"
						>
							<span
			...
			<div class="regen-modal-body">
				<div>
					...
					<p class="mb-3 text-xs text-zinc-500">
						Choose which image to rotate from. The angle will be calculated
						automatically.
					</p>
					<div class="grid grid-cols-3 gap-2">
						{#each availableSources8() as source (source.key)}
							<button
								onclick={() => (regenerateSourceDirection = source.key)}
								class="source-card {regenerateSourceDirection === source.key
									? 'source-card-active'
									: ''}"
							>
								{#if source.url}
								...
								{#if regenerateSourceDirection === source.key}
									<div class="source-card-check">
										<Check class="h-3 w-3 text-zinc-900" />
									</div>
								{/if}
							</button>
						{/each}
				...
				<div
					class="flex items-center justify-between rounded-lg border border-zinc-700/30 bg-zinc-800/30 px-3 py-2"
				>
					<span class="text-zinc-400">Rotation Angle</span>
					<span class="font-medium text-white">
						{calculateAngle8(
							regenerateSourceDirection,
							regenerateTargetDirection,
						)}°
					</span>
				</div>

				<div
					class="flex items-center justify-between rounded-lg border border-amber-500/15 bg-amber-500/5 px-3 py-2"
				>
					<span class="text-sm text-zinc-300">Cost</span>
					<span class="text-sm font-semibold text-amber-400"
						>{SINGLE_VIEW_TOKEN_COST} tokens</span
					>
				</div>
			</div>
...
<style>
	...
	.dir-cell-filled:hover .dir-cell-hover {
		opacity: 1;
	}
