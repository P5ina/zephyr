/**
 * Credit invariants for the single-view regenerate on the rotate-new path.
 *
...
function regenerate(user: UserSnapshot | null): Promise<Response> {
	return (POST as unknown as (e: RegenerateEvent) => Promise<Response>)({
		params: { id: JOB_ID },
		request: new Request(
			`http://localhost/api/rotate-new/${JOB_ID}/regenerate`,
			{
				method: 'POST',
...
describe('POST /api/rotate-new/[id]/regenerate — credit', () => {
	...
	it('spends exactly one view worth of credit on one request', async () => {
		...
		expect((await jobRow()).rotationRight).toBe(
...
describe('POST /api/rotate-new/[id]/regenerate — refund', () => {
	...
	// token_cost is the original four-view generation, and the user still holds
	// the asset.
