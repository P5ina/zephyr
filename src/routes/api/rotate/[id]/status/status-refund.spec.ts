/**
 * Money invariants for the 8-direction rotation status poller.
 *
 ...
 *
 * The second test is the cross-endpoint case: the user presses Cancel while a
 * poll is in flight. `/api/rotate/[id]/cancel` claims the job atomically and
 * refunds it; this handler then resumes holding a row that still says
 * 'processing'. It must not pay a second time. That interleaving is forced
 * deterministically here — the cancel runs inside the awaited fal.ai call —
 * rather than hoped for from parallel dispatch.
 */
import {
	afterAll,
	beforeAll,
...
vi.mock('$lib/server/fal', () => ({
	getRotation8DirJobStatus: vi.fn(async () => {
...
const USER_ID = 'user-rot-1';
const GUEST_ID = 'guest-rot-1';
const JOB_ID = 'rot-job-1';
...
function poll(locals: Event['locals'] = { user: { id: USER_ID } }) {
	return (status.GET as unknown as Handler)({
		params: { id: JOB_ID },
		locals,
		url: new URL(`http://localhost/api/rotate/${JOB_ID}/status`),
	});
}
...
function cancelAsOwner() {
	return (cancel.POST as unknown as Handler)({
		...
		url: new URL(`http://localhost/api/rotate/${JOB_ID}/cancel`),
	});
}
...
beforeEach(async () => {
	...
	await ctx.db.insert(table.rotationJob).values({
		...
		userId: USER_ID,
		status: 'processing',
		runpodJobId: FAL_REQUEST_ID,
		tokenCost: TOKEN_COST,
		bonusTokenCost: BONUS_TOKEN_COST,
	});
});

describe('GET /api/rotate/[id]/status — refund on fal.ai failure', () => {
	...
	it('does not refund a job that already completed', async () => {
		await ctx.db
			...
			.set({ status: 'completed', rotationS: 'https://cdn.test/s.png' })
			.where(eq(table.rotationJob.id, JOB_ID));
	...
	it('does not deliver results over a failed job', async () => {
		fal.reply = {
			status: 'COMPLETED',
			output: { s: 'https://cdn.test/s.png', n: 'https://cdn.test/n.png' },
		};
		fal.during = cancelAsOwner;
		...
		expect(row.rotationS).toBeNull();
		expect(await balance()).toBe(TOKEN_COST);
	});
});

describe('GET /api/rotate/[id]/status — guest-owned job', () => {
	beforeEach(async () => {
		await ctx.db
			.delete(table.rotationJob)
			.where(eq(table.rotationJob.id, JOB_ID));
