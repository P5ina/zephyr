...
export const POST: RequestHandler = async ({ params, request, locals }) => {
	...
	const job = await db.query.rotationJob.findFirst({
		where: and(
			...
			eq(table.rotationJob.userId, userId),
		),
	});
	...
	// Checked before the charge: a missing key is a server misconfiguration that
	// cannot produce a view, and charging first would take the tokens on a
	// request that never reaches fal.ai.
	if (!env.FAL_KEY) {
	 ...
	 * rotation whose `token_cost` is the original eight-view generation.
	 * Claiming it would refund that larger amount and flip an asset the user
	 * already has to `failed` — and with `NOT_TERMINAL` it would in fact claim
	 * nothing at all, leaving the user charged for a view they never got.
	 *
