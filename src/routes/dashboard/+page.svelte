<script lang="ts">
	...
	async function generateKey() {
		...
		try {
			const res = await fetch('/api/dashboard/keys/generate', {
				method: 'POST',
			});
	...
	<div class="dash-content">
		...
		<div class="panel">
			...
			{#if hasAnthropicKey}
				...
				<p class="anthropic-key-hint">
					Your own API key will be returned by <code>/api/agent/config</code> instead
					of the platform key.
				</p>
			{:else}
				...
				<p class="anthropic-key-hint">
					Optional. When set, the agent config endpoint will return your key
		...
		<div class="panel">
			...
			{#if transactions.length === 0}
				<p class="text-sm text-zinc-500">No usage yet.</p>
			{:else}
				<div class="keys-table-wrap">
					<table class="keys-table">
						<thead>
						...
						<tbody>
							{#each transactions as tx (tx.id)}
								<tr>
									...
									<td>
										<span
											class="credits-delta {tx.creditsDelta < 0
												? 'credits-neg'
												: 'credits-zero'}"
										>
